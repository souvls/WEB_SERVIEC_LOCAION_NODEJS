...
class token{
    ...
    //method create token
    static getGenerateAccessToken(user_id, username, email, role, avatar){
        return jwt.sign({"user_id": user_id, "username":username,"email":email,"role":role, "avatar": avatar},process.env.ACCESS_TOKEN_KEY,{expiresIn:"1d",algorithm:"HS256" })
    }
    static getGenerateRefreshToken(user_id, username,email,role, avatar){
        return jwt.sign({"user_id": user_id, "username":username,"email":email,"role":role, "avatar": avatar},process.env.REFRESH_TOKEN_KEY,{expiresIn:"30d",algorithm:"HS256" })
    }
